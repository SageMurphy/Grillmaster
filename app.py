    ...
    if idx < len(st.session_state["generated_questions"]):
        ...
        if st.session_state["record_phase"] == "audio_playing":
            if elapsed < 5:
                ...
                st.session_state["response_file"] = None  # Ensure clean slate per question
            ...
            if remaining > 0:
                ...
                if audio_value and "response_file" not in st.session_state:
                    ...
                    if st.button("⏹️ Confirm & Next"):
                        ...
                        """
                     # Check if this question already exists in answers
                    if not any(a["question"] == question for a in st.session_state["answers"]):
...
if st.session_state.get("show_summary", False):
    ...
    def prepare_summary_for_download():
        ...
        valid_answers = [a for a in st.session_state.get("answers", []) if a.get("response") and a.get("response") != "[No response]"]

        for i, entry in enumerate(valid_answers):
            question_text_dl = entry.get("question", f"Question {i+1}")
            answer_text_dl = entry.get("response", "[No response provided]")
